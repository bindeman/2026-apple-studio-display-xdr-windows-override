...
public static class StudioXdrHidDump
{
    ...
    [DllImport("hid.dll", SetLastError = true)] static extern bool HidD_GetInputReport(SafeFileHandle HidDeviceObject, byte[] ReportBuffer, int ReportBufferLength);
        ...
        try
        {
            for (uint i = 0; ; i++)
            {
                ...
                try
                {
                    ...
                    if (!lower.StartsWith(@"\\?\hid#") || !lower.Contains("vid_05ac") || !lower.Contains("pid_1116")) continue;
    ...
    public static DeviceInfo Probe(string path)
    {
        ...
        using (var h = CreateFile(path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero))
        {
            ...
            try
            {
                ...
                AddValues(info, h, prep, caps, HIDP_REPORT_TYPE.HidP_Input, "Input");
                AddValues(info, h, prep, caps, HIDP_REPORT_TYPE.HidP_Feature, "Feature");
            }
    ...
    static void AddValues(DeviceInfo info, SafeFileHandle h, IntPtr prep, HIDP_CAPS caps, HIDP_REPORT_TYPE reportType, string label)
    {
        ushort count = reportType == HIDP_REPORT_TYPE.HidP_Input ? caps.NumberInputValueCaps : caps.NumberFeatureValueCaps;
        if (count == 0) return;

        var vals = new HIDP_VALUE_CAPS[count];
        int status = HidP_GetValueCaps(reportType, vals, ref count, prep);
        if (status != HIDP_STATUS_SUCCESS) return;

        for (int n = 0; n < count; n++)
        {
            var cap = vals[n];
            ushort usage = cap.IsRange ? cap.UsageMin : cap.Usage;
            var value = new ValueInfo();
            value.Type = label;
            value.UsagePage = cap.UsagePage;
            value.Usage = usage;
            value.UsageMax = cap.IsRange ? cap.UsageMax : usage;
            value.ReportId = cap.ReportID;
            value.BitSize = cap.BitSize;
            value.ReportCount = cap.ReportCount;
            value.LogicalMin = cap.LogicalMin;
            value.LogicalMax = cap.LogicalMax;
            value.PhysicalMin = cap.PhysicalMin;
            value.PhysicalMax = cap.PhysicalMax;
            value.Units = cap.Units;
            value.UnitsExp = cap.UnitsExp;

            int reportLength = reportType == HIDP_REPORT_TYPE.HidP_Input ? caps.InputReportByteLength : caps.FeatureReportByteLength;
            if (reportLength > 0)
            {
                byte[] report = new byte[reportLength];
                report[0] = cap.ReportID;
                bool got = reportType == HIDP_REPORT_TYPE.HidP_Input ? HidD_GetInputReport(h, report, report.Length) : HidD_GetFeature(h, report, report.Length);
                if (got)
                {
                    uint current;
                    int getStatus = HidP_GetUsageValue(reportType, cap.UsagePage, 0, usage, out current, prep, report, report.Length);
                    value.Current = getStatus == HIDP_STATUS_SUCCESS ? current.ToString() : "status 0x" + getStatus.ToString("X8");
                }
                else
                {
                    value.Current = "read failed " + Marshal.GetLastWin32Error();
                }
            }
...
$devices = @([StudioXdrHidDump]::Enumerate())
if ($devices.Count -eq 0) {
    ...
    foreach ($instance in $instances) {
        ...
        $devices += [StudioXdrHidDump]::Probe($path)
    }
}

Write-Host "Studio Display XDR HID collections: $($devices.Count)"
foreach ($device in $devices) {
    Write-Host ""
    Write-Host "Path: $($device.Path)"
    if ($device.Error) {
        Write-Host "  Error: $($device.Error)"
        continue
    }

    Write-Host ("  Collection UsagePage=0x{0:X4} Usage=0x{1:X4} InputLength={2} FeatureLength={3}" -f $device.UsagePage, $device.Usage, $device.InputLength, $device.FeatureLength)
    foreach ($value in $device.Values) {
        Write-Host ("  {0,-7} Report=0x{1:X2} UsagePage=0x{2:X4} Usage=0x{3:X4}..0x{4:X4} Bits={5} Count={6} Logical={7}..{8} Physical={9}..{10} Units=0x{11:X8} Exp=0x{12:X8} Current={13}" -f $value.Type, $value.ReportId, $value.UsagePage, $value.Usage, $value.UsageMax, $value.BitSize, $value.ReportCount, $value.LogicalMin, $value.LogicalMax, $value.PhysicalMin, $value.PhysicalMax, $value.Units, $value.UnitsExp, $value.Current)
    }
}
