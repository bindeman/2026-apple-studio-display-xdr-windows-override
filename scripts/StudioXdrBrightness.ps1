...
public static class XdrHidBrightness
{
    ...
    [DllImport("hid.dll")]
    static extern int HidP_GetValueCaps(HIDP_REPORT_TYPE ReportType, [Out] HIDP_VALUE_CAPS[] ValueCaps, ref ushort ValueCapsLength, IntPtr PreparsedData);

    [DllImport("hid.dll")]
    static extern int HidP_GetUsageValue(HIDP_REPORT_TYPE ReportType, ushort UsagePage, ushort LinkCollection, ushort Usage, out uint UsageValue, IntPtr PreparsedData, byte[] Report, int ReportLength);

    [DllImport("hid.dll")]
    static extern int HidP_SetUsageValue(HIDP_REPORT_TYPE ReportType, ushort UsagePage, ushort LinkCollection, ushort Usage, uint UsageValue, IntPtr PreparsedData, byte[] Report, int ReportLength);

    public static List<DeviceInfo> Enumerate(string[] productIds)
    {
        Guid hidGuid;
        ...
        try
        {
            for (uint i = 0; ; i++)
            {
                ...
                try
                {
                    ...
                    bool supportedProduct = false;
                    foreach (string productId in productIds)
                    {
                        if (lower.Contains("pid_" + productId.ToLowerInvariant()))
                        {
                            supportedProduct = true;
                            break;
                        }
                    }
                    if (!supportedProduct) continue;

                    found.Add(ProbePath(path));
                }
                finally
                {
    ...
    public static DeviceInfo ProbePath(string path)
    {
        ...
        using (var h = CreateFile(path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero))
        {
            if (h.IsInvalid)
            {
                info.Error = "CreateFile failed: " + Marshal.GetLastWin32Error();
                return info;
            }

            IntPtr prep;
            if (!HidD_GetPreparsedData(h, out prep))
            {
                info.Error = "HidD_GetPreparsedData failed: " + Marshal.GetLastWin32Error();
                return info;
            }
            ...
            try
            {
                ...
                {
                    info.Error = "HidP_GetCaps failed: 0x" + status.ToString("X8");
                    return info;
                }
                ...
                if (count == 0)
                {
                    info.Error = "No feature value caps";
                    return info;
                }
                ...
                if (status != HIDP_STATUS_SUCCESS)
                {
                    info.Error = "HidP_GetValueCaps failed: 0x" + status.ToString("X8");
                    return info;
                }
                ...
                for (int n = 0; n < count; n++)
                {
                    ...
                    if (vals[n].UsagePage == 0x0082 && usage == 0x0010)
                    {
                        chosen = n;
                        break;
                    }
                ...
                if (chosen < 0)
                {
                    info.Error = "No VESA brightness feature cap found";
                    return info;
                }
                ...
                if (!HidD_GetFeature(h, report, report.Length))
                {
                    info.Error = "HidD_GetFeature failed: " + Marshal.GetLastWin32Error();
                    return info;
                }
                ...
                if (status != HIDP_STATUS_SUCCESS)
                {
                    info.Error = "HidP_GetUsageValue failed: 0x" + status.ToString("X8");
                    return info;
                }
    ...
    public static void SetBrightness(string path, uint value)
    {
        using (var h = CreateFile(path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero))
        {
            ...
            try
            {
                ...
                if (cap.LogicalMax > cap.LogicalMin && (value < cap.LogicalMin || value > cap.LogicalMax))
                    throw new ArgumentOutOfRangeException("value", "Value must be within logical min/max");
...
$normalizedProductIds = @($ProductId | ForEach-Object { $_.Trim().ToLowerInvariant() -replace '^0x', '' -replace '^pid_', '' })
$devices = [XdrHidBrightness]::Enumerate($normalizedProductIds)
if (-not $devices -or $devices.Count -eq 0) {
    $hidInterfaceGuid = "{4d1e55b2-f16f-11cf-88cb-001111000030}"
    $devices = @()
    $productPattern = ($normalizedProductIds | ForEach-Object { "PID_$($_.ToUpperInvariant())" }) -join "|"
    $instances = Get-CimInstance Win32_PnPEntity |
        Where-Object { $_.DeviceID -match "^HID\\VID_05AC&($productPattern)" } |
        Select-Object -ExpandProperty DeviceID

    foreach ($instance in $instances) {
        $path = "\\?\" + ($instance.ToLowerInvariant() -replace "\\", "#") + "#" + $hidInterfaceGuid
        $devices += [XdrHidBrightness]::ProbePath($path)
    }
}

if (-not $devices -or $devices.Count -eq 0) {
    Write-Error "No supported Apple display HID paths found for PID(s): $($normalizedProductIds -join ', ')"
}

$usable = $devices | Where-Object { -not $_.Error -and $_.UsagePage -eq 0x82 -and $_.Usage -eq 0x10 } | Select-Object -First 1

Write-Host "== Studio Display XDR HID brightness =="
Write-Host "Scanning Apple product PID(s): $($normalizedProductIds -join ', ')"
foreach ($d in $devices) {
    if ($d.Error) {
        Write-Host "Path: $($d.Path)"
        Write-Host "  Error: $($d.Error)"
    } else {
        $displayMin = if ($d.LogicalMax -gt $d.LogicalMin) { $d.LogicalMin } else { 0 }
        $displayMax = if ($d.LogicalMax -gt $d.LogicalMin) { $d.LogicalMax } else { 100000 }
        $pct = [math]::Round((($d.CurrentValue - $displayMin) * 100.0) / ($displayMax - $displayMin), 1)
        Write-Host "Path: $($d.Path)"
        Write-Host ("  UsagePage=0x{0:X4} Usage=0x{1:X4} ReportId=0x{2:X2} Range={3}..{4} Current={5} (~{6}%)" -f $d.UsagePage, $d.Usage, $d.ReportId, $displayMin, $displayMax, $d.CurrentValue, $pct)
    }
}

if (-not $usable) {
    Write-Error "No usable brightness feature found."
}
...
$fallbackMin = 0
$fallbackMax = 100000
$rangeMin = if ($usable.LogicalMax -gt $usable.LogicalMin) { $usable.LogicalMin } else { $fallbackMin }
$rangeMax = if ($usable.LogicalMax -gt $usable.LogicalMin) { $usable.LogicalMax } else { $fallbackMax }

$target = -1
if ($SetPercent -ge 0) {
    if ($SetPercent -gt 100) { throw "SetPercent must be 0..100" }
    $target = [int][math]::Round($rangeMin + (($rangeMax - $rangeMin) * ($SetPercent / 100.0)))
} elseif ($SetRaw -ge 0) {
    $target = $SetRaw
} elseif ($StepPercent -ne 0) {
    $currentPercent = [int][math]::Round((($usable.CurrentValue - $rangeMin) * 100.0) / ($rangeMax - $rangeMin))
    $nextPercent = [Math]::Max(0, [Math]::Min(100, $currentPercent + $StepPercent))
    $target = [int][math]::Round($rangeMin + (($rangeMax - $rangeMin) * ($nextPercent / 100.0)))
    Write-Host ""
    Write-Host "Current brightness is approximately $currentPercent%; stepping to $nextPercent%."
}

if ($target -ge 0) {
    if ($target -lt $rangeMin -or $target -gt $rangeMax) {
        throw "Target brightness must be between $rangeMin and $rangeMax."
    }
    Write-Host ""
    Write-Host "Setting brightness raw value to $target..."
    [XdrHidBrightness]::SetBrightness($usable.Path, [uint32]$target)
    Write-Host "Done. Rerun with -ListOnly to confirm."
}
